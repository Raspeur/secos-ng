...
- `0x600000` : PGD du kernel (physique)
- `0x601000` : PTB (kernel) pour 0..4MB
- `0x602000` : PTB2 (kernel) pour 4..8MB
- `0x603000` : PGD pour user1 (première tâche utilisateur)
- `0x604000` : PTB pour user1 (mappage 0..4MB)
- `0x605000` : PTB2 pour user1 (mappage 4..8MB)
- `0x606000` : PGD pour user2 (deuxième tâche utilisateur)
- `0x607000` : PTB pour user2 (mappage 0..4MB)
- `0x608000` : PTB2 pour user2 (mappage 4..8MB)
...
- `Task_Context[i].cr3` contient l'adresse physique complète du PGD (ex. `0x603000` ou `0x606000`).
...
- Les deux tâches ont un mapping virtuel identique pour 0..8MB (identity mapping des pages), mais leurs tables de pages (PGD/PTB) résident physiquement à des endroits différents (`0x603000` vs `0x606000`).
- La page partagée : phys `0x700000` est mappée virtuellement pour chaque tâche à des adresses virtuelles différentes (`0x700000` pour task A, `0x701000` pour task B).
